...
Quatre formats, même source (chiffres = payload, **pas** recalculés. Facts tels quels, pas de `message`) :

| Format | Contenu |
|---|---|
| **JSON** | `{ export_version: 1, kind: "planning", team, restaurant_name, week_labels, employees` (fiches équipe, **sans** `invite_token`), `assignments, facts, stats, legal_cols, legal_rows, wish_cols, wish_rows, score }` |
