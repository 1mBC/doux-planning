# core-2.3: titulaires jusqu'à 48h légales (concentrate hours, not contract cap)
...
def _can_repair_fill_titular(
    ...
    """Check if employee can fill this window (titular mode: cap = 48h legal)."""
    weekday = WEEKDAYS[day_index % 7]
...
def _repair_score_titular(
    ...
    """Compute repair score for tie-breaking (titular mode: concentrate hours)."""
    weekday = WEEKDAYS[day_index % 7]
    structure = draft.structure_for(team, service_id, weekday)
    assigned = _assigned_window(employee, window, structure)
    
    day_shifts = [s for s in assignments if s.employee_id == employee.id and s.day_index == day_index]
    already_on_day = 0 if day_shifts else 1
    
    week_hours = _hours_in_week(assignments, employee.id, day_index)
    hours_inverse = -week_hours
    
    overqual = employee.level - window.level
    
    return (already_on_day, hours_inverse, overqual, employee.id)
...
def _repair_titular(
    ...
    """Attempt to fill empty posts with titular mode (48h cap, concentrate)."""
    ...
    for day_index, service_id, team, window in empty_windows:
        ...
        for employee in team_employees:
            if not _can_repair_fill_titular(draft, assignments, off_days, employee, day_index, service_id, team, window):
                skipped_illegal += 1
                continue
            
            score = _repair_score_titular(draft, assignments, employee, day_index, service_id, team, window)
            structure = draft.structure_for(team, service_id, weekday)
            assigned = _assigned_window(employee, window, structure)
...
def generate_cycle(draft: PlanningDraft, search: SearchEffort | None = None) -> EngineResult:
    ...
    def consider(off_days: dict[str, set[int]]) -> None:
        ...
        repairs = {"attempted": empty_before, "filled": 0, "remaining": empty_before, "skipped_illegal": 0, "mode": "titular48"}
        result = pre_repair_result
        
        if empty_before > 0:
            repaired_assignments = list(assignments)
            repairs = _repair_titular(draft, repaired_assignments, off_days)
