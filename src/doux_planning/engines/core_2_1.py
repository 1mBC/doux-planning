# core-2.1: fill fewest + repair empty posts
# Based on core_2.py (f34ff3b2d0e998c4fa568f4ca3d0d7161c3bab72) with repair pass.
...
# core-2.1: hours beyond contract accepted during repair pass
...
def _count_empty_posts(result: EngineResult) -> int:
    """Count empty_post warnings in result."""
...
def _creates_coupure_on_day(assignments: list[Shift], employee_id: str, day_index: int, new_shift: Shift) -> bool:
    """Check if adding new_shift would create a coupure (gap) on this day."""
    day_shifts = [
        s for s in assignments
        if s.employee_id == employee_id and s.day_index == day_index
    ]
    if not day_shifts:
        return False
    return _has_gap(day_shifts + [new_shift])
...
def _repair_score(
    ...
    has_coupure = 1 if _creates_coupure_on_day(assignments, employee.id, day_index, trial) else 0
...
def _repair_empty_posts(draft: PlanningDraft, assignments: list[Shift]) -> int:
    """Attempt to fill empty posts. Returns count of posts filled."""
    filled_count = 0
    ...
    for day_index, service_id, team, window in empty_windows:
        ...
        for employee in team_employees:
            if not _can_repair_fill(draft, assignments, employee, day_index, service_id, team, window):
                continue
...
def generate_cycle(draft: PlanningDraft, search: SearchEffort | None = None) -> EngineResult:
    ...
    empty_before = _count_empty_posts(best)
    filled_count = 0
    
    if empty_before > 0:
        assignments = list(best.assignments)
        filled_count = _repair_empty_posts(draft, assignments)
        if filled_count > 0:
            best = evaluate(draft.with_assignments(tuple(assignments)))
    
    empty_after = _count_empty_posts(best)
