# core-2.6: copy of core-2. Balanced weekends, contract choice, surplus rests, exact window.
# Live types: Shift, PlanningDraft, EngineResult, Candidate.
...
from doux_planning.engine import Candidate, EngineResult, PlanningDraft, Shift
...
def _assigned_window(
    ...
    """core-2.6 keeps the service-type window. No stretch toward the minimum."""
    del employee, structure
    return window
...
def _window_meets_min_shift(
    ...
    need = int(min_shift_for(employee, structure.service_id) * 60)
    return window.end_minutes - window.start_minutes >= need
...
def _can_fill_window(
    ...
    *,
    ...
    if _would_exceed_coupures(assignments, employee, trial):
        return False
    # core-2.6: no veto when some other colleague is still under contract.
    del employee_pool
    penalty = _soft_penalty(draft, assignments, employee, trial)
    return penalty[0] == 0


def _contract_ratio(employee: Employee, hours: float) -> float:
    contract = employee.contractual_hours_per_week
    if contract > 0:
        return hours / contract
    return 0.0 if hours <= 0 else float("inf")
...
def _pick_for_post(
    ...
    *,
    ...
    """Prefer staff whose minimum fits, then those under contract, then the lowest ratio."""
    del remaining_windows
    structure = draft.structure_for(team, service_id, weekday)
    if structure is None:
        return None
    ...
    if not available:
        return None
    fit = [employee for employee in available if _window_meets_min_shift(employee, hole, structure)]
    pool = fit if fit else available
    under = [
        employee
        for employee in pool
        if _hours_in_week(assignments, employee.id, day_index) + 1e-9 < employee.contractual_hours_per_week
    ]
    choix = under if under else pool

    def sort_key(employee: Employee) -> tuple:
        hours = _hours_in_week(assignments, employee.id, day_index)
        return (_contract_ratio(employee, hours), employee.level - window_level, employee.id)

    return min(choix, key=sort_key), hole
...
def _weekend_off_side(draft: PlanningDraft) -> dict[str, str]:
    """Off side for each weekend wish. every_two takes the lighter side, A on a tie."""
    count_a = 0
    count_b = 0
    every: list[Employee] = []
    locked: dict[str, str] = {}
    for employee in draft.employees:
        choice = employee.wellbeing.weekend
        if choice is WeekendChoice.EVEN:
            locked[employee.id] = "A"
            count_a += 1
        elif choice is WeekendChoice.ODD:
            locked[employee.id] = "B"
            count_b += 1
        elif choice is WeekendChoice.EVERY_TWO:
            every.append(employee)
    for employee in sorted(every, key=lambda item: item.id):
        if count_a <= count_b:
            locked[employee.id] = "A"
            count_a += 1
        else:
...
def _day_is_closed(draft: PlanningDraft, day_index: int) -> bool:
    weekday = WEEKDAYS[day_index % 7]
    return all(draft.hours.is_closed(weekday, service_id) for service_id in draft.hours.services)


def _day_surplus(
    draft: PlanningDraft, off: dict[str, set[int]], day_index: int, person_id: str
) -> int:
    """Min across the day's services of coworkers who can hold a post, minus posts."""
    weekday = WEEKDAYS[day_index % 7]
    gaps: list[int] = []
    for service_id in draft.hours.services:
        if draft.hours.is_closed(weekday, service_id):
        ...
        for team in Team:
            ...
            posts += len(windows)
            for employee in draft.employees:
                if employee.id == person_id:
                    continue
                if day_index in off.get(employee.id, set()):
                    continue
                if employee.team != team:
                    continue
                if any(employee.level >= window.level for window in windows):
                    capable.add(employee.id)
        if posts == 0:
            continue
        gaps.append(len(capable) - posts)
    if not gaps:
        return 0
    return min(gaps)


def _surplus_rest_days(draft: PlanningDraft) -> dict[str, set[int]]:
    """After weekend locks, place each owed rest on the open day with the highest surplus."""
    off: dict[str, set[int]] = {employee.id: set() for employee in draft.employees}
    for day in range(CYCLE_DAYS):
        if _day_is_closed(draft, day):
            for employee in draft.employees:
                off[employee.id].add(day)
    for employee in draft.employees:
        off[employee.id].update(employee.forced_off_days)
    sides = _weekend_off_side(draft)
    must_work: dict[str, set[int]] = {employee.id: set() for employee in draft.employees}
    for employee in draft.employees:
        side = sides.get(employee.id)
        if side == "A":
            off[employee.id].update((5, 6))
            must_work[employee.id].update((12, 13))
        elif side == "B":
            off[employee.id].update((12, 13))
            must_work[employee.id].update((5, 6))
    for employee in sorted(draft.employees, key=lambda item: item.id):
        for week_start in (0, 7):
            already = sum(1 for day in range(week_start, week_start + 7) if day in off[employee.id])
            owed = REST_DAYS_PER_WEEK - already
            for _rest in range(max(0, owed)):
                best_day: int | None = None
                best_key: tuple[int, int] | None = None
                for day in range(week_start, week_start + 7):
                    if day in off[employee.id] or day in must_work[employee.id]:
                        continue
                    if _day_is_closed(draft, day):
                        continue
                    key = (_day_surplus(draft, off, day, employee.id), -day)
                    if best_key is None or key > best_key:
                        best_key = key
                        best_day = day
                if best_day is None:
                    break
                off[employee.id].add(best_day)
    return off


def _build_rest_model(
    draft: PlanningDraft, *, hard_coverage: bool, balance: bool = True
    ...
    sides = _weekend_off_side(draft) if balance else {}
    work: dict[tuple[str, int], cp_model.IntVar] = {}
    for employee in draft.employees:
        for day in range(CYCLE_DAYS):
    ...
    for employee in draft.employees:
        ...
        if employee.wellbeing.weekend is not None:
            ...
            choice = employee.wellbeing.weekend
            if balance and choice is WeekendChoice.EVERY_TWO:
                side = sides[employee.id]
                if side == "A":
            ...
            elif choice is WeekendChoice.EVEN:
                model.Add(even_off == 1)
                model.Add(odd_off == 0)
            else:
                model.Add(odd_off == 1)
...
def _slack_or_fallback(
    draft: PlanningDraft, seconds: float, *, balance: bool = False
) -> dict[str, set[int]]:
    slack, slack_work, unders = _build_rest_model(draft, hard_coverage=False, balance=balance)
    if unders:
...
def _rebalance_hours(
    draft: PlanningDraft,
    assignments: list[Shift],
    off_days: dict[str, set[int]],
    employee_pool: list[Employee],
) -> None:
    """Move one shift from the fullest to the lightest colleague while the gap exceeds 0.5h."""
    groups: dict[tuple[int, float], list[Employee]] = {}
    for employee in employee_pool:
        groups.setdefault((employee.level, employee.contractual_hours_per_week), []).append(employee)
    for week_start in (0, 7):
        for group in groups.values():
            ...
            while True:
                hours = {
                    employee.id: _hours_in_week(assignments, employee.id, week_start) for employee in group
                }
                gap = max(hours.values()) - min(hours.values())
                if gap <= 0.5 + 1e-9:
                    break
                max_hours = max(hours.values())
                min_hours = min(hours.values())
                givers = [employee for employee in group if abs(hours[employee.id] - max_hours) <= 1e-9]
                receivers = [employee for employee in group if abs(hours[employee.id] - min_hours) <= 1e-9]
                best_rank: tuple | None = None
                best_move: tuple[Employee, Shift] | None = None
                for giver in givers:
                    for receiver in receivers:
                        if giver.id == receiver.id:
                            continue
                        for shift in assignments:
                            if shift.employee_id != giver.id:
                                continue
                            if not week_start <= shift.day_index < week_start + 7:
                                continue
                            duration = shift.duration_hours
                            giver_after = hours[giver.id] - duration
                            receiver_after = hours[receiver.id] + duration
                            if receiver_after > giver_after + 1e-9:
                                continue
                            if not _receiver_can_hold_shift(
                                draft, assignments, shift, receiver, off_days, employee_pool
                            ):
                                continue
                            updated = dict(hours)
                            updated[giver.id] = giver_after
                            updated[receiver.id] = receiver_after
                            new_gap = max(updated.values()) - min(updated.values())
                            reduction = gap - new_gap
                            if reduction <= 1e-9:
                                continue
                            rank = (-reduction, shift.day_index, shift.start_minutes, giver.id, receiver.id)
                            if best_rank is None or rank < best_rank:
                                best_rank = rank
                                best_move = (receiver, shift)
                if best_move is None:
                    break
                receiver, shift = best_move
                index = assignments.index(shift)
                assignments[index] = replace(shift, employee_id=receiver.id)
...
def _fill_assignments(
    ...
    _rebalance_hours(draft, assignments, off_days, employee_pool)
    return assignments
