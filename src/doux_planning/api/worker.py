...
def _stale_cutoff() -> datetime:
    return _now() - timedelta(seconds=STALE_HEARTBEAT_SECONDS)


def _touch_heartbeat(model, job_id: str) -> None:
...
def _heartbeat(stop: threading.Event, started: float, job_id: str, team: str) -> None:
    while not stop.wait(PROGRESS_EVERY_S):
        _touch_heartbeat(GenerateJob, job_id)
        iso_log(
            "generate progress",
            job_id=job_id,
            team=team,
...
def _bench_heartbeat(stop: threading.Event, started: float, job_id: str) -> None:
    while not stop.wait(PROGRESS_EVERY_S):
        _touch_heartbeat(BenchJob, job_id)
        iso_log(
            "bench progress",
            job_id=job_id,
            elapsed_s=round(time.perf_counter() - started, 1),
...
def _reclaim_table(model, event: str) -> int:
...
def reclaim_stale_running_jobs() -> int:
    return _reclaim_table(GenerateJob, "job requeued")


def reclaim_stale_bench_jobs() -> int:
    return _reclaim_table(BenchJob, "bench job requeued")


def reclaim_stale_jobs() -> int:
    return reclaim_stale_running_jobs() + reclaim_stale_bench_jobs()
...
def tick_generate_job(*, generate_team_fn: GenerateFn | None = None) -> str | None:
    ...
    with session_scope() as db:
        ...
        team_value = job.team
    iso_log("job taken", job_id=job_id, team=team_value, restaurant_id=restaurant_id, worker=_worker_id())
    iso_log("generate start", job_id=job_id, team=team_value, rss_mb=_rss_mb(), worker=_worker_id())
...
def tick_bench_job(*, run_bench_fn: GenerateFn | None = None) -> str | None:
    ...
    try:
        ...
        stop.set()
        beat.join(timeout=0.2)
    iso_log("bench job end", job_id=job_id, status="done", run_id=row.id, worker=_worker_id())
    _set_bench_job(job_id, "done", run_id=row.id)
    return job_id


def run_worker_loop(*, idle_seconds: float = 1.0) -> None:
    iso_log("worker start", pid=os.getpid(), rss_mb=_rss_mb(), worker=_worker_id())
    reclaim_stale_jobs()
    while True:
        reclaim_stale_jobs()
