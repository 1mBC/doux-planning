...
Load the four frozen salle bench datasets from disk and run isolated generation against a human oracle, without touching a live restaurant or keep-best.
...
#### Scenario: Four complete datasets
- **WHEN** the repo contains the four frozen salle datasets
- **THEN** `list_bench_datasets` returns four entries
...
- **WHEN** each of the four datasets’ expected assignments is evaluated
