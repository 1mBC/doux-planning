...
- `GET /v1/admin/bench/datasets`, `GET /v1/admin/bench/runs`, `POST /v1/admin/bench/run`, `GET /v1/admin/bench/jobs/{id}`, `GET /v1/admin/bench/runs/{id}`, `GET /v1/admin/bench/compare/{category}/{dataset_id}/{search_effort}`

Boot after seed: if `ADMIN_EMAIL` matches an existing restaurateur (lowercase), set `is_admin`. Skip when unset/empty or when no restaurateur row exists — never insert an account. `me` always includes `admin` (company from `is_admin`, employee always false). `kind` stays company|employee. Log generate on HTTP 200 and worker job `done` (`email`, `restaurant_name`, `team`, `search_effort`, `duration_seconds` wall-clock of `generate_team`, `warnings` of the team just solved with each item = HTTP warning plus `employee_name` from the fiche at log time, else null). `GET /v1/admin/generates` always emits `search_effort` and `duration_seconds` (null on old rows). Alembic adds `restaurateur_accounts.is_admin`, `generate_logs`, and nullable `generate_logs.search_effort` / `generate_logs.duration_seconds`.

Admin bench (`contracts/domain/bench.md`) wraps Core `list_bench_datasets` / `load_bench_dataset` / `run_bench` / `UnknownBenchDataset`. Disposable state only — never write `published_cycles` or `generate_logs`. `app_version` is the trimmed `data/bench/VERSION` line persisted on each run. `POST /v1/admin/bench/run` with `scope=dataset` and `minimal`/`optimized` is a 200 `{ runs: [RunSummary] }` and one `bench_runs` row. `all`, `category`, or `maximal` is 202 `{ job_ids, status: queued }` with one `bench_jobs` row per listed dataset (`all` = 7 today; `category=crafted` = 3). Routes unchanged — enqueue from `list_bench_datasets`. The same worker loop claims `bench_jobs` (`SKIP LOCKED`) independently of `generate_jobs` (no cross 409). A `done` job always inserts `bench_runs`. Failed job → `error` FR, no run. Alembic tables `bench_jobs` and `bench_runs` already exist (no new migration). SPA fallback `/admin`, `/admin/bench`, `/admin/bench/{category}/{dataset_id}/{search_effort}`. Pytest ticks an exported bench function with `run_bench` stubbed (0 s). Do not edit `engine.py` or `data/bench/**`.
