...
`weekend_rest_day` is part of that same JSONB object (no Alembic). Parse missing key as Core default `false`; GET always emits the bool. Do not coerce old Railway rows that still store a key list — those stay 400. `GET /v1/me/planning` must pass through Core `BoardWish` `kind: "weekend_rest_day"` when the box is posed (no kind filter). Legacy list key `at_least_one_weekend_rest_day` stays 400.
