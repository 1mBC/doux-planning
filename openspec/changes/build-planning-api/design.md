...
Admin bench (`contracts/domain/bench.md`) wraps Core `list_bench_datasets` / `load_bench_dataset` / `run_bench` / `UnknownBenchDataset`. Disposable state only — never write `published_cycles` or `generate_logs`. `app_version` is the trimmed `data/bench/VERSION` line persisted on each run. `POST /v1/admin/bench/run` with `scope=dataset` and `minimal`/`optimized` is a 200 `{ runs: [RunSummary] }` and one `bench_runs` row. `all`, `category`, or `maximal` is 202 `{ job_ids, status: queued }` with one `bench_jobs` row per dataset. The same worker loop claims `bench_jobs` (`SKIP LOCKED`) independently of `generate_jobs` (no cross 409). A `done` job always inserts `bench_runs`. Failed job → `error` FR, no run. Alembic tables `bench_jobs` and `bench_runs` (JSONB scores / assignments). SPA fallback `/admin`, `/admin/bench`, `/admin/bench/{category}/{dataset_id}/{search_effort}`. Pytest ticks an exported bench function with `run_bench` stubbed (0 s). Do not edit `engine.py` or `data/bench/**`.
