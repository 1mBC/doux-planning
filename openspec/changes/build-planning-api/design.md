...
- `GET /v1/admin/bench/datasets`, `GET /v1/admin/bench/runs`, `POST /v1/admin/bench/run`, `GET /v1/admin/bench/jobs/{id}`, `GET /v1/admin/bench/runs/{id}`, `GET /v1/admin/bench/compare/{category}/{dataset_id}/{search_effort}`, `GET /v1/admin/bench/export`
...
Admin bench (`contracts/domain/bench.md`) wraps Core `list_bench_datasets` / `load_bench_dataset` / `run_bench` / `UnknownBenchDataset`. Disposable state only — never write `published_cycles` or `generate_logs`. `app_version` is the trimmed `data/bench/VERSION` line persisted on each run. `POST /v1/admin/bench/run` with `scope=dataset` and `minimal`/`optimized` is a 200 `{ runs: [RunSummary] }` and one `bench_runs` row. `all`, `category`, or `maximal` is 202 `{ job_ids, status: queued }` with one `bench_jobs` row per listed dataset (`all` = 7 today; `category=crafted` = 3). Routes unchanged — enqueue from `list_bench_datasets`. The same worker loop claims `bench_jobs` (`SKIP LOCKED`) independently of `generate_jobs` (no cross 409). A `done` job always inserts `bench_runs`. Failed job → `error` FR, no run. GET compare returns the last-run summary plus `employees` and `model` / `manual` `CycleSlice` recomputed via Core `cycle_recap_from_draft` (persisted assignments + `expected.json` + context). It MUST NOT emit top-level `facts` / `assignments` / `expected` aliases. GET `/v1/admin/bench/export` (`scope=dataset` needs `category` + `dataset_id`; `scope=below_manuel`) returns `{ export_version: 1, kind: "bench-pack", app_version, exported_at, scope, datasets }` with catalogue `context` (no `invite_token`), `manual` slice, and every last-run effort (`minimal` → `maximal`) for included jeux. `below_manuel` includes a jeu when at least one last-run has `score.global < expected_score.global` (both non-null) and packs **all** run efforts of those jeux; empty → 200 `datasets: []`. Dataset without a run → 404. No Alembic for recap (recompute). Alembic tables `bench_jobs` and `bench_runs` already exist (no new migration). SPA fallback `/admin`, `/admin/bench`, `/admin/bench/{category}/{dataset_id}/{search_effort}`. Pytest ticks an exported bench function with `run_bench` stubbed (0 s). Do not edit `engine.py` or `data/bench/**`.
