...
- `GET /v1/admin/bench/datasets`, `GET /v1/admin/bench/runs`, `POST /v1/admin/bench/run`, `GET /v1/admin/bench/jobs/{id}`, `GET /v1/admin/bench/runs/{id}`, `GET /v1/admin/bench/compare/{category}/{dataset_id}/{search_effort}`, `GET /v1/admin/bench/export`, `GET /v1/admin/bench/versions`
...
Admin bench (`contracts/domain/bench.md`) wraps Core `list_bench_datasets` / `load_bench_dataset` / `run_bench` / `engine_ref` / `UnknownBenchDataset`. Disposable state only — never write `published_cycles` or `generate_logs`. Persist writes `bench_runs.app_version` = `outcome.engine_ref` (no separate VERSION read). HTTP summaries, datasets, and export emit `engine_ref` and `app_version` as the same string; stored `"0.27.0"` reads as `"core-0"`. Last-run is newest per `(category, dataset_id, search_effort, engine_ref)`. `POST /v1/admin/bench/run` with `scope=dataset` and `minimal`/`optimized` is a 200 `{ runs: [RunSummary] }` and one `bench_runs` row. `all`, `category`, or `maximal` is 202 `{ job_ids, status: queued }` with one `bench_jobs` row per listed dataset (`all` = 50 today; `category=crafted` = 26; enqueue idempotent on the partial unique key). Routes unchanged — enqueue from `list_bench_datasets`. GET datasets is 50 jeux with current `engine_ref` (`core-2`). Worker replicas share the same image: heartbeat every 10 s, reclaim only stale (`heartbeat_at` NULL or older than 180 s) at start and each loop. Alembic adds `heartbeat_at` plus the bench unique index. The same worker loop claims `bench_jobs` (`SKIP LOCKED`) independently of `generate_jobs` (no cross 409). A `done` job always inserts `bench_runs`. Failed job → `error` FR, no run. GET compare path returns the **current** `engine_ref` last-run plus `employees` and `model` / `manual` `CycleSlice` recomputed via Core `cycle_recap_from_draft`. GET `/runs/{id}` is the same shape for that row (no top-level `assignments` / `facts`). GET `/versions` is the matrix (`engine_refs` merged, three effort keys). GET `/v1/admin/bench/export` uses current last-runs only and adds `engine_ref` / `run_id`. `below_manuel` includes a jeu when at least one **current** last-run has `score.global < expected_score.global` (both non-null) and packs **all** current run efforts of those jeux; empty → 200 `datasets: []`. Dataset without a current run → 404. No Alembic for recap / `engine_ref`. Alembic tables `bench_jobs` and `bench_runs` already exist (no new migration). SPA fallback `/admin`, `/admin/bench`, `/admin/bench/versions`, `/admin/bench/run/{run_id}`, `/admin/bench/{category}/{dataset_id}/{search_effort}`. Pytest ticks an exported bench function with `run_bench` stubbed (0 s). Do not edit `engine.py` or `data/bench/**`.
