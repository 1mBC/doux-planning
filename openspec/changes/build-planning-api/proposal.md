...
- `POST /v1/generate` synchrone → `generate_team` → 200 `{ team, search_effort, published }`. `GET /v1/cycles` relit le persisté. Chaque cycle non null porte le `CycleRecap` Core (`stats`, `legal_*`, `wish_*`) via `cycle_recap` — pas de chiffres inventés dans `api/`. Persist JSONB, pas d’Alembic. Cycle déjà persisté sans recap : GET hydrate + `cycle_recap`. Publish live sandbox = même `published`. Sandbox **live** : `/v1/live/sandbox/{team}/…` wrappe Core enter/preview/apply/undo/discard/publish, persist `live_sandboxes` (pas la table joujou). `GET /v1/me/planning` (Bearer employee) wrappe `employee_board` — grille publiée de son équipe, pas `/me/shifts`. Pas de table `jobs`, pas de worker. evaluate / swap / rank restent plus tard.
