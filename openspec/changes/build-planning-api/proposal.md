...
- Admin (`contracts/domain/admin.md`) : boot `ADMIN_EMAIL` promote un restaurateur **existant** (`is_admin`, idempotent, aucun insert). `me.admin` bool. `POST /v1/generate` 200 → une ligne `generate_logs`. `GET /v1/admin/generates` Bearer admin, newest-first. Non-admin → 403. Pas de `kind: admin`. Alembic `is_admin` + `generate_logs`.
...
- 3 slots de cycle (`contracts/domain/generate-versions.md`) : `published[team] = { versions: { minimal, optimized, maximal }, latest }`. Chaque cycle porte recap + `generated_at` + `search_effort`. POST / job done écrit **ce** slot + `latest`. Coerce plat ancien → `versions.optimized`. `GET /v1/me/planning` = `versions[latest]`. Enter `search_effort` (défaut latest) ; publish même slot, `generated_at` inchangé. Logs stdout ISO (worker + uvicorn 202 / done / failed). Pas d’Alembic (JSONB). Exemple 92.
