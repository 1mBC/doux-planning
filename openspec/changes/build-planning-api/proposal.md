...
- `POST /v1/generate` hybride C (`contracts/domain/generate-jobs.md`) : `minimal` / `optimized` (omis) restent **200** sync `generate_team` + `generate_logs`. `maximal` → **202** `{ job_id, team, search_effort, status: queued, estimated_seconds: 600 }` sans solve dans uvicorn. `GET /v1/generate/jobs/{id}` même resto (`published` ssi `done`). Worker Compose `SKIP LOCKED` → persist cycles + log si succès. `TeamNotReady` → 409, aucun job. 2ᵉ maximal queued/running même team → 409. Alembic `generate_jobs`. Tests = tick stub (0 s), pas 600 s. `GET /v1/cycles` relit le persisté. Recap Core inchangé. Sandbox live / `GET /v1/me/planning` inchangés. evaluate / swap / rank restent plus tard.
...
- `GET /v1/context/export` / `POST /v1/context/import` (Bearer company) : JSON portable `export_version: 1` (`contracts/domain/export-config.md`). Export strip `company_code` / `invite_token`. Import smash comme seed (cycles / linked / comptes salariés) + name du JSON + tokens neufs. Version ≠ 1 → 400. Clés interdites ignorées. Pas de generate. Exemple public 92 inchangé.
- Admin (`contracts/domain/admin.md`) : boot `ADMIN_EMAIL` promote un restaurateur **existant** (`is_admin`, idempotent, aucun insert). `me.admin` bool. `POST /v1/generate` 200 / job `done` → une ligne `generate_logs` avec `search_effort`, `duration_seconds` (wall-clock `generate_team`), et chaque warning HTTP **plus** `employee_name` (fiche à cet instant, sinon null). `GET /v1/admin/generates` Bearer admin, newest-first, expose ces clés. Vieux rows : `search_effort` / `duration_seconds` **null**. Non-admin → 403. Pas de `kind: admin`. Alembic `is_admin` + `generate_logs` + 2 colonnes nullable (`search_effort`, `duration_seconds`).
- Lecture Railway (`contracts/domain/coerce-railway.md`) : GET context / export / generate / me/planning **coercent** l’ancien JSONB wellbeing (liste de clés, clés retirées, `every_*`, `service_id` null, `max_*_per_week` sur la fiche) vers l’objet Core, puis **soignent** la ligne si le JSONB diffère. PATCH / import restent 400 `Champs invalides.` (pas de coerce à l’écriture). `[]` / absent → `Wellbeing()`. Objet déjà Core inchangé. Pas d’Alembic. Exemple public 92 inchangé.
- Jobs Maximal : table `generate_jobs` (Alembic), Compose service `worker` (`python -m doux_planning.api.worker`), même image Railway sans domaine public. Pas de Redis/Celery.
- 3 slots de cycle (`contracts/domain/generate-versions.md`) : `published[team] = { versions: { minimal, optimized, maximal }, latest }`. Chaque cycle porte recap (y compris `score` = Core `cycle_recap.score`, forme `contracts/domain/score.md` avec `notes` + `resumes` + `global` + `weights`) + `generated_at` + `search_effort` + `duration_seconds` (wall-clock du solve ; **absent** sur les vieux slots). POST / job done / GET cycles / hydrate recap manquant émettent `score`. Vieux JSONB sans `score` ou `score` sans `resumes` : GET calcule via Core (comme recap), pas 500, pas d’objet partiel. GET exemple public peut omettre `score`. Coerce plat ancien → `versions.optimized`. `GET /v1/me/planning` = `versions[latest]`. Enter `search_effort` (défaut latest) ; publish même slot, `generated_at` / `duration_seconds` inchangés. Logs stdout ISO (worker + uvicorn 202 / done / failed). Pas d’Alembic (JSONB). Exemple 92. Keep-best / jobs / versions inchangés. Clé JSON `contrat` inchangée.
- Un seul chemin de score : appeler le moteur, sérialiser `EngineResult`. Pas de React. Pas de modification du moteur hors `api/` sauf blocage.
