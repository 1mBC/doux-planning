...
- 3 slots de cycle (`contracts/domain/generate-versions.md`) : `published[team] = { versions: { minimal, optimized, maximal }, latest }`. Chaque cycle porte recap + `generated_at` + `search_effort` + `duration_seconds` (wall-clock du solve ; **absent** sur les vieux slots). POST / job done écrit **ce** slot + `latest`. Coerce plat ancien → `versions.optimized`. `GET /v1/me/planning` = `versions[latest]`. Enter `search_effort` (défaut latest) ; publish même slot, `generated_at` / `duration_seconds` inchangés. Logs stdout ISO (worker + uvicorn 202 / done / failed). Pas d’Alembic (JSONB). Exemple 92.
