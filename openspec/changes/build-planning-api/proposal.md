...
- 3 slots de cycle (`contracts/domain/generate-versions.md`) : `published[team] = { versions: { minimal, optimized, maximal }, latest }`. Chaque cycle porte recap (y compris `score` = Core `cycle_recap.score`, forme `contracts/domain/score.md`) + `generated_at` + `search_effort` + `duration_seconds` (wall-clock du solve ; **absent** sur les vieux slots). POST / job done / GET cycles / hydrate recap manquant émettent `score`. Vieux JSONB sans `score` : GET calcule via Core (comme recap), pas 500. GET exemple public peut omettre `score`. Coerce plat ancien → `versions.optimized`. `GET /v1/me/planning` = `versions[latest]`. Enter `search_effort` (défaut latest) ; publish même slot, `generated_at` / `duration_seconds` inchangés. Logs stdout ISO (worker + uvicorn 202 / done / failed). Pas d’Alembic (JSONB). Exemple 92. Keep-best / jobs / versions inchangés.
