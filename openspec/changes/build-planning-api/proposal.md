...
- `POST /v1/generate` synchrone → `generate_team` → 200 `{ team, search_effort, published }`. `GET /v1/cycles` relit le persisté. Sandbox **live** : `/v1/live/sandbox/{team}/…` wrappe Core enter/preview/apply/undo/discard/publish, persist `live_sandboxes` (pas la table joujou). Pas de table `jobs`, pas de worker. evaluate / swap / rank et `/me/shifts` restent plus tard.
