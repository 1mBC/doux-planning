...
Uniquement `web/`. Proxy Vite `/v1` inchangé. API = uvicorn `master` + worker Compose. Version `0.29.0`. Hors slice : rotate invite-token, delete salarié, édition des poids, plusieurs `engine_ref`, rewrite `saint-cloud.json`.
