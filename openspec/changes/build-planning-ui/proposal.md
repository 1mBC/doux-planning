...
Uniquement `web/`. Proxy Vite `/v1` inchangé. L’API recaps tourne à part (ne pas merger `recaps/infra` / `recaps/core`). Types = JSON du contrat ; recap 5 clés requises sur un cycle non null. Pas de nouvelle dépendance. Version `0.15.0`. Hors slice : rotate invite-token, `optimized` 30 s, edit contraintes salarié.
