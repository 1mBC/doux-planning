...
- `kind: company` : route `/context` (lien « Mon restaurant ») — wizard selon `contracts/domain/wizard-ui.md` : **Services → Rôles → Équipe → Souhaits → Services types → Semaine type**. Services types : **une ligne** chronologique, horloge + ±15 collés, **STAFF après cette arrivée / ce départ** en en-tête seulement, pire-cas → `remaining_post_levels`. Rôles : Nom + Niveau stepper. Invite : bouton **Inviter mes employés** (QR + URL), plus de jeton sous les fiches. Décocher un service = warning + purge. `weekend_rest_day` à côté de la radio we, sans sous-texte. Bearer sur GET/PATCH `/v1/context` et `POST /v1/context/seed-example`.
...
Uniquement `web/`. Proxy Vite `/v1` inchangé. L’API alerts tourne à part (ne pas merger `alerts/infra` / `alerts/core`). Types = JSON du contrat. Petite lib QR OK. Version `0.16.0`. Hors slice : rotate invite-token, `optimized` 30 s, exports, edit contraintes salarié.
