...
- `kind: company` : route `/context` (lien « Mon restaurant ») — wizard selon `contracts/domain/wizard-ui.md` : **Services → Rôles → Équipe → Souhaits → Services types → Semaine type**. Services types : **une `<table>` par feuille**, colonnes Type · Heure · **Niveaux minimal requis (par arrivée | après sortie)** · STAFF (plus de colonne N). Stepper **encadré**, libellé gras, chiffre centré. Invite : URL **absolue** + QR. `weekend_rest_day` en **colonne** à part. Bearer sur GET/PATCH `/v1/context` et `POST /v1/context/seed-example`.
...
Uniquement `web/`. Proxy Vite `/v1` inchangé. API = uvicorn `master` + worker Compose. Version `0.23.0`. Hors slice : rotate invite-token.
