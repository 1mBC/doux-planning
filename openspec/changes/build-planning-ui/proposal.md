...
- `kind: company` : route `/context` (lien « Mon restaurant ») — wizard selon `contracts/domain/wizard-ui.md` : **Services → Rôles → Équipe → Souhaits → Services types → Semaine type**. Décocher un service = warning FR puis purge (types, semaine, indispos, `max_services`) des deux équipes. Service non offert invisible. Souhaits : case **`weekend_rest_day`** à côté de la radio we (cumulable). Services types : sous-onglets par service, vagues en pickers d’échelle (pire-cas → `remaining_post_levels`). Salle et cuisine indépendantes. `ready.*` affiché tel quel. Bearer sur GET/PATCH `/v1/context` et `POST /v1/context/seed-example`. `week_labels` lecture seule.
...
Uniquement `web/`. Proxy Vite `/v1` inchangé. L’API weekend-rest tourne à part (ne pas merger `weekend-rest/infra` / `weekend-rest/core`). Types = JSON du contrat ; `weekend_rest_day` bool requis ; ancienne clé liste → throw. Pas de nouvelle dépendance. Version `0.14.0`. Hors slice : rotate invite-token, `optimized` 30 s, edit contraintes salarié.
