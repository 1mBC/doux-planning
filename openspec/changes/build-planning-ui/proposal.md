...
- `kind: company` : route `/context` (lien « Mon restaurant ») — wizard contexte selon `contracts/http/v1-context.md` : identité (nom PATCH, légal France lecture seule, `company_code`) puis rôles → **Équipe** → **Souhaits bien-être** → services → types → semaine type. « Fiches » disparaît. Salle et cuisine indépendantes. `ready.*` affiché tel quel (badges FR). Bearer sur GET/PATCH `/v1/context` et `POST /v1/context/seed-example` en plus de l’auth. Toujours pas sur exemple / sandbox. `week_labels` lecture seule (`ab` → A/B, `parity` → Paire/Impaire).
...
Uniquement `web/`. Proxy Vite `/v1` inchangé. L’API seed tourne à part (ne pas merger `seed/infra` / `seed/core`). Types = JSON du contrat ; clé manquante → throw. Pas de nouvelle dépendance. Version `0.13.0`. Hors slice : rotate invite-token, `optimized` 30 s, edit contraintes salarié.
