...
- `kind: employee` : pas de Mode édition ; phrase FR (planning publié personnel plus tard). Pas de wizard `/context` (403 API).
- `kind: company` : route `/context` (lien « Mon restaurant ») — wizard contexte selon `contracts/http/v1-context.md` : identité (nom PATCH, légal France lecture seule, `company_code`) puis rôles → fiches → services → types → semaine type. Salle et cuisine indépendantes. `ready.*` affiché tel quel (badges FR). Aucun generate. Bearer sur GET/PATCH `/v1/context` en plus de l’auth. Toujours pas sur exemple / sandbox.
...
- `planning-ui`: application web du snapshot d’exemple, login / register / session, et wizard contexte entreprise.
...
Uniquement `web/`. Proxy Vite `/v1` inchangé. L’API contexte tourne à part (ne pas merger `context/infra` / `context/core`). Types = JSON du contrat ; clé manquante → throw. Pas de nouvelle dépendance. Version `0.8.0`. Hors slice : rotate invite-token, generate, grille employé colorée.
