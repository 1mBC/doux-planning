...
- `kind: company` : route `/planning` selon `v1-generate.md` + `generate-versions.md` + `generate-jobs.md` + `cycle-recaps.md` + `score.md` : **3 rangées** — Salle/Cuisine ; Minimal/Optimisé/Maximal = **sélection** (défaut `latest`, slot vide → « Pas encore calculé ») ; actions blanches **(Re)Calculer le planning** / **Entrer en mode édition** / **Quitter le mode édition** / **Publier** / **Exporter**. Recalculer = POST de l’effort sélectionné. Édition : enter avec cet effort. Sous la rangée 3 : `generated_at` Europe/Paris puis **durée** `duration_seconds` (absent → tiret). Rangée **notes /10** : **Globale à gauche** (cadre contrasté, pas de résumé) puis 5 axes ; jauge horizontale `note / 10` ; `resumes` en `pre-line` ; Occupation /10 ; HSL `hue = 12 × note`. Plus de cartes stats. `LegalRecap` / `WishRecap` sous la grille. Loader ≥ 1 s. Maximal = 202 + poll. Parse `published[team].versions` + `latest`. Salarié : `me/planning` `latest` seulement, pas de sélecteur ni notes.
...
- `me.admin` (bool, employee → `false`) : lien **Admin** dans le chrome ssi `true`. Route `/admin` : admin → `GET /v1/admin/generates` table (en-têtes jour Paris, colonnes **Effort** + **Durée**, hover = **une carte par warning** comme le recap planning) **et** lien **Banc** vers `/admin/bench` ; sinon message `Action réservée à l’admin.` sans fetch. Vide → « Aucun generate pour l’instant. » Pas de bouton salarié / `/exemple` / `/planning`.
- `me.admin` : `/admin/bench` selon `bench.md` — `GET datasets` + `GET runs` ; lancer all | catégorie | jeu × Minimal / Optimisé / Maximal ; tableau une ligne par jeu, 3 efforts = dernier run (globale · oracle · Δ) ; clic → `/admin/bench/{category}/{id}/{effort}` (généré puis oracle, notes, grilles lecture). `dataset`+minimal|optimized = 200 ; all|category|maximal = 202 + poll jobs ou refresh runs. Non-admin : message habituel, 0 appel bench.
...
Uniquement `web/`. Proxy Vite `/v1` inchangé. API = uvicorn `master` + worker Compose. Version `0.28.0`. Hors slice : rotate invite-token, delete salarié, édition des poids, plusieurs `engine_ref`, rewrite `saint-cloud.json`.
