...
Uniquement `web/`. Proxy Vite `/v1` inchangé. L’API wellbeing tourne à part (ne pas merger `wellbeing/infra` / `wellbeing/core`). Types = JSON du contrat ; clé manquante → throw. Pas de nouvelle dépendance. Version `0.12.0`. Hors slice : rotate invite-token, `optimized` 30 s, bouton seed, edit contraintes salarié.
