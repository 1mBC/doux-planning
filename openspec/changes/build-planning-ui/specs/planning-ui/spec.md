...
`/admin/bench` SHALL use a single header row: one column per `engine_refs` value, with no `colSpan` and no second header row of Minimal | Optimisé | Maximal. Each model cell SHALL stack three deltas vertically in that order, each with a small effort label; a click SHALL open `/admin/bench/run/{run_id}`. Per-dataset Launch and the Défi column SHALL follow Requirement: Admin bench chrome. The global Launch toolbar rows SHALL stay horizontal. Manuel, recap, loader, gaps, and launch `locked = busy || exporting` stay unchanged except as that requirement moves the global exports.

#### Scenario: One column per engine_ref with three stacked deltas
- **WHEN** an admin opens `/admin/bench`
- **THEN** `core-3` is a single column whose cell shows Minimal, Optimisé, and Maximal deltas stacked
...
### Requirement: Import restaurant to bench
`/admin` SHALL open a modal **Au banc** from a generate row via (1) right-click on the **restaurant name** (`preventDefault`) and (2) a button **Au banc** in the Planning cell next to **Voir**. Right-click on **email** MUST keep the impersonate copy gesture and MUST NOT open this modal. If `restaurant_id` is null the client SHALL toast `Restaurant introuvable.`, MUST NOT open the modal, and MUST NOT GET preview or POST import. The modal header SHALL show `restaurant_name` and `email` from the row. The client SHALL `GET /v1/admin/restaurants/{id}/import-preview` with Bearer and render a **complet ?** panel: per team (`salle`, `cuisine`) `ready`, `manuel_published`, `computes_published` (`minimal` / `optimized` / `maximal`), plus `generate_count`. An HTTP error or unexpected body SHALL show API `detail` (or the parse message) inside the modal and MUST NOT crash `/admin`. Four checkboxes SHALL default **checked**: **Salle**, **Cuisine**, **Dernier planning manuel publié**, **Computes déjà publiés**. An optional **Note manuel /10** number input (0–10, empty = no override; the UI MUST NOT force 0,1) SHALL show help « Si le planning à la main n'est pas complet. » An optional **Commentaire** textarea SHALL be offered. Actions SHALL be **Annuler** (close) and **Importer**. **Importer** SHALL `POST /v1/admin/bench/import` with Bearer and JSON `{ restaurant_id, include_salle, include_cuisine, include_manuel, include_runs, manual_score, comment }` sending the four booleans **explicitly** from the checkboxes. Empty note SHALL send `manual_score: null`. Empty/whitespace comment SHALL send `comment: null`. On 200 the client SHALL close the modal and toast `Jeu importé.` with a button/link to `/admin/bench`. On error the modal SHALL show `detail`. `parseBenchVersions` SHALL read dataset `origin` (`catalogue` | `imported`) and `comment` (`string | null`) when present; missing keys SHALL default `origin: "catalogue"` and `comment: null` and MUST NOT throw. `/admin/bench` table chrome SHALL follow Requirement: Admin bench chrome. Version bar SHALL be `0.57.0` with note `Import resto vers le banc` until that requirement. File 70 (Note / Voir / right-click email) MUST stay unchanged.

#### Scenario: Right-click restaurant name opens the import popup
- **WHEN** an admin right-clicks a restaurant name whose row has `restaurant_id`
- **THEN** a modal opens with four checkboxes checked, and the client GETs import-preview

#### Scenario: Au banc button opens the same popup
- **WHEN** an admin clicks **Au banc** next to **Voir**
- **THEN** the same modal opens

#### Scenario: Null restaurant_id does not open the popup
- **WHEN** `restaurant_id` is null and the admin right-clicks the restaurant name or clicks **Au banc**
- **THEN** the client toasts `Restaurant introuvable.` and does not GET preview or POST import

#### Scenario: Right-click email still copies impersonate
- **WHEN** an admin right-clicks the email
- **THEN** the client POSTs impersonate, copies `url`, and does not open the import modal

#### Scenario: Import success toasts and links to banc
- **WHEN** POST `/v1/admin/bench/import` returns 200
- **THEN** the modal closes and the toast `Jeu importé.` offers a control that goes to `/admin/bench`

#### Scenario: Preview error shows detail
- **WHEN** GET import-preview fails with an API `detail`
- **THEN** that `detail` is shown in the modal and `/admin` still renders

#### Scenario: Versions parser defaults origin and comment
- **WHEN** `GET /v1/admin/bench/versions` omits `origin` and `comment` on a dataset
- **THEN** the parser sets `origin` to `catalogue` and `comment` to `null` and the page still renders

### Requirement: Admin bench chrome
`/admin/bench` SHALL keep the Launch block (engine select, all-categories × three efforts, per-compute category dropdowns, **Compléter les trous**, inline batch loader) and SHALL move **Exporter sous le Manuel** and **Exporter tout le banc** into that Launch block. Under the **Derniers runs** `h2` and before the table, the client SHALL show filter buttons **Tous | IA | Manuels**, default **Tous**. The filter SHALL be client-side on `dataset.origin` (`catalogue` = IA, `imported` = Manuels). An empty filtered list SHALL show « Aucun jeu. » The table SHALL keep Catégorie, Jeu, Manuel, and `engine_ref` columns. It MUST NOT render a **Défi** `th` or a **Lancer** `th`, and MUST NOT render a per-row LaunchButtons / Exporter pile. The Jeu cell SHALL show `id` and `name`. Hovering that text SHALL show `challenge_fr` via native `title` (or an existing admin-tip tooltip); if `comment` is non-null the client SHALL append it. Under the name the client SHALL show a **`…`** button that opens a menu closed on outside click or Escape, with exactly: Lancer Minimal, Lancer Optimisé, Lancer Maximal, Exporter ce jeu, Supprimer… Launch and export SHALL use the same POST/GET as today. API `detail` SHALL be shown on error. **Supprimer…** SHALL `window.confirm('Supprimer ce jeu et tous ses résultats ?')` then `DELETE /v1/admin/bench/datasets/{category}/{id}` with Bearer. HTTP 204 SHALL refresh `GET /v1/admin/bench/versions`. If Infra is not landed or the response is an error, the client SHALL show `detail`. Stats banc SHALL stay unchanged (no `…` menu). Historique (Note / Voir / right-click email) and the **Au banc** popup SHALL stay unchanged until Requirement: Admin UI pass. Version bar SHALL be `0.58.0` with note `Menu jeu, filtre IA / manuels` until Requirement: Admin UI pass.

#### Scenario: Table has no Défi or Lancer columns
- **WHEN** an admin opens `/admin/bench`
- **THEN** the table headers do not include Défi or Lancer, Launch is not a per-row stack, and global exports sit in the Launch block

#### Scenario: Origin filter hides catalogue rows
- **WHEN** versions lists catalogue and imported datasets and the admin clicks **Manuels**
- **THEN** only `origin=imported` rows remain; if none, the copy is « Aucun jeu. »

#### Scenario: Row menu launches, exports, and deletes
- **WHEN** an admin opens `…` on a dataset
- **THEN** the menu lists Lancer Minimal, Lancer Optimisé, Lancer Maximal, Exporter ce jeu, and Supprimer… ; confirming delete DELETEs that dataset and a 204 refreshes `/versions`

### Requirement: Admin UI pass
Follow `contracts/domain/admin-ui-pass.md` UI (wins over historique table chrome and the in-page origin filter). `/admin` column order SHALL be Heure, Email, **Actions**, Restaurant, Équipe, Effort, **Note**, Durée, Moteur. The table MUST NOT render **Warnings** or **Planning**, MUST NOT render **Voir** or **Au banc**, and MUST NOT attach `onContextMenu` on email or restaurant. **Actions** (immediately after Email) SHALL be two `choice` buttons labeled exactly `impersonate` and `exporter vers le banc`. If `restaurant_id` is null both SHALL be disabled and the client MUST NOT POST. `impersonate` SHALL mint the copyable link (`POST /v1/admin/impersonate`), copy `url`, and toast `Lien copié — ouvre-le en navigation privée.` On failure it SHALL toast `detail`. The client MUST NEVER `window.open`. `exporter vers le banc` SHALL open the existing import popup (preview, 4 checkboxes, optional note /10, comment, Importer). After a successful import the toast action **Ouvrir le banc** SHALL go to `/admin/bench/manuels` (not `/admin/bench`). **Note** SHALL be a button `.bench-cell` wrapping a span `.bench-delta-bubble` whose text is `formatCycleNote(score_global)`. Bubble color SHALL use exported `noteHue` / `noteTint` from recaps (`hue = 12 * note`) and MUST NOT use `deltaBackground`. A null score SHALL render the encart without tint and text `—`. Clicking the note SHALL `go("/admin/planning/" + restaurant_id)` when `restaurant_id` is non-null; otherwise the button SHALL be disabled. Hovering **the note wrapper/button only** SHALL show the existing `FactTip` (`.admin-tip`); empty facts SHALL show `aucun warning`. The note MUST NOT have a native `title`. CSS MUST NOT use `.admin-table tbody tr:hover .admin-tip`. Engine selector, AdminPlanningPage, and ImpersonateScreen SHALL stay unchanged. Admin nav labels SHALL be **Historique des computes | Banc IA | Banc Manuels | Stats banc**. `AdminNavCurrent` SHALL be `"history" | "bench" | "bench-manuels" | "bench-stats"`. Banc IA SHALL go to `/admin/bench`. Banc Manuels SHALL go to `/admin/bench/manuels`. There MUST NOT be a single **Banc** button. `Root` SHALL map `/admin/bench/manuels` to the same `BenchPage` with `origin` forced by the path, matching that path **before** the unknown `/admin/*` compare fallback. `/admin/bench` and `/admin/bench/versions` (redirect to `/admin/bench`) SHALL stay Banc IA. On run/compare pages neither Banc nav button SHALL be `disabled`. One `BenchPage` SHALL set `h1` to **Banc IA** or **Banc Manuels**, filter datasets client-side with `dataset.origin ===` the page origin, and show « Aucun jeu. » when empty. The in-page **Tous | IA | Manuels** filter MUST NOT render. The `…` menu, Lancer block, global exports, engine select, and loader SHALL stay. Banc IA category launcher SHALL list catalogue categories only (exclude `imported`). Banc Manuels SHALL hide the per-category launch row; **Toutes les catégories** SHALL still launch all imported. `postBenchRun` for scope all | category | gaps SHALL always include `origin` of the page. `loadBenchExport` for `below_manuel` and `bank` SHALL pass `origin` as a query. `scope=dataset` launches/exports SHALL omit `origin`. Version bar SHALL be `0.59.0` with note `Note colorée, actions, deux pages de banc`.

#### Scenario: Historique columns and Actions
- **WHEN** an admin opens `/admin`
- **THEN** the table headers are Heure, Email, Actions, Restaurant, Équipe, Effort, Note, Durée, Moteur, and the Actions cell has `impersonate` and `exporter vers le banc`

#### Scenario: Null restaurant disables Actions and note click
- **WHEN** a generate row has `restaurant_id` null
- **THEN** both Actions buttons and the note button are disabled and the client does not POST impersonate or import

#### Scenario: Note hover is encart-only
- **WHEN** an admin hovers the note encart (and not the rest of the row)
- **THEN** `FactTip` is shown; hovering another cell on the same row does not show it

#### Scenario: Banc IA hides imported datasets
- **WHEN** an admin opens `/admin/bench` and versions include catalogue and imported datasets
- **THEN** the `h1` is Banc IA, only `origin=catalogue` rows render, and Tous | IA | Manuels is absent

#### Scenario: Banc Manuels hides catalogue and category launcher
- **WHEN** an admin opens `/admin/bench/manuels`
- **THEN** the `h1` is Banc Manuels, only `origin=imported` rows render, and the per-category launch row is hidden

#### Scenario: Compare keeps both Banc nav buttons clickable
- **WHEN** an admin is on a run or compare page
- **THEN** neither Banc IA nor Banc Manuels is `disabled`
