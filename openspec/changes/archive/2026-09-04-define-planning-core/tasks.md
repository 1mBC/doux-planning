...
- [x] 4.20 Store min_shift_hours on the employee (default 4), pair posts FIFO when remaining skill coverage allows, stretch a shorter window to that minimum within the service span, and verify a 3h closer is filled at 4h when the service continues, and left empty when it cannot
