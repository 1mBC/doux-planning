...
Shift fill is sequential per post window (highest required level first, then earliest start). Post windows are paired FIFO when remaining coverage still allows it: among people who can leave, the earliest arrival leaves first; a higher-skill opener stays while remaining posts still need that skill. When assigning a later post, skip a candidate who is the only person left who can cover an earlier still-empty window of the same service, if someone else can take the later post (so a 10:00 opening is not sacrificed for a 12:00 level-2). Eligible staff: same team, level ≥ post, not unavailable, no overlap, assigned shift at least as long as that employee’s min_shift_hours (default 4, per profile). If the coverage window is shorter, the shift is stretched to that minimum (end first, then start) within the service span — overstaffing the tail is preferred to an empty post. A second service the same day does not authorize a shorter shift. Choice order: lowest hours-to-contract ratio, then already on that day, then nearest level. A shift that would exceed contractual hours is skipped when any teammate is still under hours. Unavailability remains a hard skip. A shift that would exceed that person’s weekly coupure cap is a hard skip (same as unavailability): empty post or another colleague, never a 3rd midi+soir when the cap is 2. Legal maxima (11h rest, 5h pause, daily/weekly caps) skip a candidate when another eligible person exists. Finishing a started day still applies until the coupure cap is reached.
