...
- Emit `CycleScore.resumes`: the same five keys, `null` iff the note is `null`, exact FR forms from `contracts/domain/score.md` (hours `{label} occupées / {label} contrat`, indispo on the next line, roles `{N} affectés · {k} poste en sous-rôle / {N}`). Notes /10 stay unchanged.
